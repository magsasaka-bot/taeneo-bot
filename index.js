...
const { SocksProxyAgent } = require('socks-proxy-agent'); // Added for SOCKS5 support
...
function normalizeProxyUrl(proxy) {
  if (!proxy.startsWith('http://') && !proxy.startsWith('https://') && !proxy.startsWith('socks5://')) {
    proxy = 'http://' + proxy; // Default to HTTP if no protocol is specified
  }
  return proxy;
}
...
function displayAccountData(index) {
  console.log(chalk.green(`================= Account ${index + 1} =================`));
  ...
  console.log(chalk.green(`Points Total: ${chalk.greenBright.bold(pointsTotals[index])}`));
  console.log(chalk.green(`Points Today: ${chalk.greenBright.bold(pointsToday[index])}`));
  console.log(`Message: ${chalk.yellow(messages[index])}`);
...
function logAllAccounts() {
  ...
  console.log("\nStatus:");
  for (let i = 0; i < accounts.length; i++) {
    console.log(`${chalk.whiteBright(`Account ${i + 1}`)}: ${chalk.magenta(`Potential Points: ${potentialPoints[i]}`)}, ${chalk.magenta(`Countdown: ${countdowns[i]}`)}`);
  }
}
...
async function connectWebSocket(index) {
  ...
  if (useProxy && proxy) {
    if (proxy.startsWith('socks5://')) {
      agent = new SocksProxyAgent(normalizeProxyUrl(proxy)); // SOCKS5 proxy support
    } else {
      agent = new HttpsProxyAgent(normalizeProxyUrl(proxy)); // HTTP or HTTPS proxy support
    }
  }
